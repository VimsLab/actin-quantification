...
from preprocess import visualize_offset
...
#################
    ...
    instance_mask = img[crop_edge:img.shape[0] - crop_edge, crop_edge : img.shape[1] - crop_edge]


    # import pdb; pdb.set_trace()
    mask_pologons = mask_to_pologons(instance_mask)

    if len(mask_pologons) == 0:
        return end_points, off_sets_next, control_points, off_sets_prev, mask_pologons
    ...
    if len(intersections):
        ...
        for idx, contour in enumerate(contours):
            #####################################################################
            # canvas_debug = cv2.drawContours(canvas_debug, contours, idx, 1, 1)ad
    ...
    return end_points, control_points, off_sets_prev, off_sets_next, mask_pologons
    ...
    if file_exist==False and no_ori==False:
        ...
        for img_id in tqdm(coco_ids):
            ...
            for idx, ann in enumerate(anns):
                ...
                end_point, control_point, off_sets_prev, off_sets_next, mask_pologons= get_keypoints(mask, this_image, step = 50, crop_edge = 25, debug = False)
            ...
            crop_edge = 25
            overlapping_area = overlapping_area[crop_edge:overlapping_area.shape[0] - crop_edge, crop_edge : overlapping_area.shape[1] - crop_edge]
...
#################################################################################################
    ...
    #     # target_file = 'ann_keypoint_instance_mask.json'
    #     target_file = 'ann_keypoint_instance_mask_test.json'
    #     # import pdb; pdb.set_trace()

    #     trans_anno(img_root, anno_root, output_root, ori_file, target_file)

# #######################################################
    root = '/usa/yliu1/colab/work/fiberPJ/data/fiber_labeled_data/instance_labeled_file'
    ...
    coco_anno_file = '/usa/yliu1/colab/work/fiberPJ/data/fiber_labeled_data/fb_coco_style_fifth_test.json'
    # coco_anno_file = '/home/yiliu/work/fiberPJ/data/fiber_labeled_data/skel_5_coco_style_fifth.json'
    # show_anno(root, coco_anno_file)
    anno_root = '/usa/yliu1/colab/work/fiberPJ/data/fiber_labeled_data/'
    output_root = anno_root
    img_root = root
